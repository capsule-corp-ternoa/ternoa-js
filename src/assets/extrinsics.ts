 ...
 * @param target        Public address of the account to transfer the amount to.
 * @param amount        Token amount to transfer.
 * @returns             Unsigned unsubmitted Assets-Transfert Transaction Hash. The Hash is only valid for 5 minutes.
 */
export const assetTransferTx = async (
  id: number,
  target: string,
  amount: number | BN,
): Promise<TransactionHashType> => {
  const transferredAmount = typeof amount === "number" ? await numberToBalance(amount) : amount
  return await createTxHex(txPallets.assets, txActions.transfer, [id, target, transferredAmount])
}
...
export const assetTransfer = async (
  ...
  const tx = await assetTransferTx(id, target, amount)
