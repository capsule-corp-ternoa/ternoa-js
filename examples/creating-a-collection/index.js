...
async function main() {
    ...
    let signedTxs = await Promise.all(signableTxs.map(tx => (0, index_3.signTx)(keyring, tx)));
    // Batching transactions allow us to execute just one transaction instead 10 of them separately. 
    let signableBatch = await (0, index_3.batchAllTxHex)(signedTxs);
    let signedBatch = await (0, index_3.signTx)(keyring, signableBatch);
    // This allows us to submit a transaction in a blocking way. The return value is a array of Events or in case of an 
    // error an exception will be thrown.
    let events = await (0, index_2.submitTxBlocking)(signedBatch, constants_1.WaitUntil.BlockInclusion);
    // Now we just need to filter the right dog events and viola, our dogs are created :) 
    let dogs = events.filter(event => event.type == constants_1.EventType.CreateNFT);
